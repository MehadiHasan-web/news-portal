...
<h1 class="mt-4">Subcategores Tables</h1>
    ...
    <div class="card-body">
        <table id="datatablesSimple">
            ...
            <tbody>
                ...
                <tr>
                    <td>{{ $data->sub_category_en }}</td>
                    <td>{{ $data->sub_category_bn }}</td>
                    <td>{{ $data->category->category_en }}  </td>
                    <td >
                        <a href="{{ route('category.edit', $data->id) }}" class="btn btn-info btn-sm "><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16">
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h1 class="modal-title fs-5" id="exampleModalLabel">Add New Subcategory</h1>
        ...
        <div class="modal-body">
            <form action="{{ route('subcategory.store') }}" method="POST"> @csrf
                <div class="mb-3">
                  ...
                  <input type="text" class="form-control @error('subcategory_en')is-invalid @enderror" id="cat_en" name="subcategory_en" required>
                ...
                <div class="mb-3">
                  ...
                  <input type="text" class="form-control @error('subcategory_en')is-invalid @enderror" id="bangla" name="subcategory_bn" required>
                    @error('subcategory_en')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                    </span>
                ...
                <div class="mb-3">
                    ...
                    <select name="category_id" class="form-select form-select-sm" aria-label=".form-select-sm example" required>
                        ...
                        @foreach ($subcategory as $data)
                            <option value="{{ $data->category->id }}">{{ $data->category->category_en }} | {{ $data->category->category_bn }}</option>
                        @endforeach
                      </select>
                  </div>
